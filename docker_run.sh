...
docker run -it --rm $GPU_OPT --env="DISPLAY=$DISPLAY" --env="QT_X11_NO_MITSHM=1" --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw" --mount source="$GRADLE_VOLUME",target=/root/.gradle -v ~/.minecraft:/ws/minecraft_ros2/run minecraft_ros2:latest 
xhost -local:root
